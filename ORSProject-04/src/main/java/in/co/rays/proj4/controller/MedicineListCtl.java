...
/**
 * MedicineListCtl handles listing, searching, pagination and bulk actions
 * for Medicine entities.
 * 
...
public class MedicineListCtl extends BaseCtl {
	...
	private static final Logger log = Logger.getLogger(MedicineListCtl.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			List<MedicineBean> list = model.search(bean, pageNo, pageSize);
			List<MedicineBean> next = model.search(bean, pageNo + 1, pageSize);
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || OP_NEXT.equalsIgnoreCase(op)
				...
				if (ids != null && ids.length > 0) {
					...
					ServletUtility.setSuccessMessage("Medicine deleted successfully", request);
				} else {
	...
	protected String getView() {
		return ORSView.MEDICINE_LIST_VIEW;
	}
}